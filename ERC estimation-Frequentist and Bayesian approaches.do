...
*Input ERC_ols
bayesmh ERC_ols, reffects(gvkey) likelihood(t(ERC_var,13)) noconstant prior({ERC_ols:i.gvkey}, normal({mu},{sig2})) prior({mu}, normal(0, 1e6)) prior({sig2}, igamma(0.001, 0.001)) block({mu}) block({sig2}) showreffects({ERC_ols:i.gvkey}) nchains(3) initall({ERC_ols:} rnormal( 0, 100) {mu} rnormal(0, 100) {sig2} runiform(0, 100)) adapt(tolerance(0.002)) mcmcsize(5000) burnin(5000) rseed(149)
...
bayesmh return i.gvkey i.gvkey#c.delta_E, likelihood(normal({var_0})) noconstant prior ({return:i.gvkey i.gvkey#c.delta_E}, mvnormal(2, {return:_cons}, {return:delta_E}, {covar,m})) block ({return: i.gvkey}, reffects) block ({return: i.gvkey#c.delta_E}, reffects) prior({var_0}, igamma(0.01, 0.01)) block({var_0}, gibbs) prior({covar,m}, iwishart(2, 3, I(2))) block({covar,m}, gibbs) prior({return:delta_E _cons}, normal(0, 1e2)) block({return:_cons}) block({return:delta_E}) nchains(3) initall({return: i.gvkey} rnormal( 0, 100) {return: i.gvkey#c.delta_E} rnormal(0, 100) {var_0} runiform(0, 100) {return:delta_E _cons} rnormal(0, 1e2)) adapt(tolerance(0.002)) mcmcsize(5000) burnin(5000) rseed(149)

*Diagnostics
bayesgraph diagnostic {return: delta_E}
bayesgraph trace {return: delta_E}
bayesgraph kdensity {return: delta_E}
bayesstats grubin
...
bayesmh return i.gvkey i.gvkey#c.delta_E ratedelta_E, likelihood(normal({var_0})) noconstant prior ({return:i.gvkey i.gvkey#c.delta_E}, mvnormal(2, {return:_cons}, {return:delta_E}, {covar,m})) block ({return: i.gvkey}, reffects) block ({return: i.gvkey#c.delta_E}, reffects) prior({var_0}, igamma(0.01, 0.01)) block({var_0}, gibbs) prior({covar,m}, iwishart(2, 3, I(2))) block({covar,m}, gibbs) prior({return: delta_E _cons ratedelta_E}, normal(0, 1e2)) block({return:_cons}) block({return:delta_E}) nchains(3) initall({return: i.gvkey} rnormal( 0, 100) {return: i.gvkey#c.delta_E} rnormal(0, 100) {var_0} runiform(0, 100) {return: delta_E _cons ratedelta_E} rnormal(0, 100)) adapt(tolerance(0.002)) mcmcsize(5000) burnin(5000) rseed(149)
